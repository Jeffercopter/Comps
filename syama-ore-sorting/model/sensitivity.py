...
import os
src = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "engine.py")).read().split("# ------------------------------------------------------------------ INPUTS")[0]
ns = {}; exec(src, ns)
cascade, solve_deportment, goal_seek = ns['cascade'], ns['solve_deportment'], ns['goal_seek']
G = 31.1035
I = dict(F=200.0, dil=0.11905973741018575, ROM=2.40, scat_frac=0.16408, scat_grade=2.00,
         hours=7370.0, rec=0.75, au_oz=4000.0, crej=7.69, csort=0.45,
         capex=4_095_000.0, disc=0.08, life=120, build=12, orec=0.90, w2r=0.90)
I['scat_tph'] = I['scat_frac']*I['F']
AU = I['au_oz']/G
...
def run(dil=None, scat_grade=None, orec=None, w2r=None, capture=1.0, inc_grade=None,
    ...
    net = d_au*I['rec']*au - I['csort']*s['peb'] - I['crej']*s['rej']
...
TABLES.append(("A.  Capacity capture  -  what share of freed SAG capacity is actually filled with ore",
    ...
    [[f"{c:.0%}"] + [run(capture=c)[k] for k in ("dROM","oz","net_a","npv")] for c in
     (0,0.10,0.25,0.40,0.50,0.75,1.00)]))
...
TABLES.append(("E.  Waste dilution  -  drives the back-solved heterogeneity contrast",
...
json.dump([[t[0], t[2], t[3]] for t in TABLES],
          open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "sensitivity_results.json"), "w"), indent=1)
